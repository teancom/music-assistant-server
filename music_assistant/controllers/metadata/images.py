...
class ImageProxyMixin:
    ...
    async def _persist_image_id(self, image_id: str, provider: str, path: str) -> None:
        ...
        try:
            ...
            # a fresh row (less than half its TTL spent) still points at this exact
            # provider/path; a changed path (e.g. a re-signed expiring URL) always
            # forces a rewrite so the mapping never serves a stale signature.
