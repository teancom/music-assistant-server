...
def _episode() -> Mock:
    media_item = Mock(spec=PodcastEpisode)
    media_item.duration = DURATION
    media_item.name = "Some Episode"
    return media_item
...
async def test_resume_uses_retained_progress_when_abs_stale(provider: Mock) -> None:
    ...
    # read-repair: we try to push the retained value, guard its socket echo, and clear it
    provider._client.update_my_media_progress.assert_awaited_once()
    provider.progress_guard.add_progress.assert_any_call(BOOK_ID, None)
    assert provider.progress_outbox.get(BOOK_ID) is None
...
@pytest.mark.asyncio
async def test_mark_episode_unplayed_does_not_recreate_zero_progress(provider: Mock) -> None:
    """Marking an episode unplayed must return after discarding local/abs progress."""
    provider._client.get_my_media_progress.return_value = None  # abs has no progress
    provider.progress_outbox.record(
        EPISODE_ID, position=1500, duration=DURATION, fully_played=False
    )
    ...
    assert provider.progress_outbox.get(EPISODE_ID) is None
    provider._client.update_my_media_progress.assert_not_awaited()
